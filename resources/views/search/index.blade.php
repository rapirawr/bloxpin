...
<div class="w-full px-2 sm:px-4 md:px-6 mx-auto pt-6" x-data="searchGallery()">
    ...
    <div class="mb-8 px-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            @if(!empty($query))
                <h1 class="text-2xl font-bold text-dark">Hasil untuk "{{ $query }}"</h1>
            @elseif($activeTag)
                <h1 class="text-2xl font-bold text-dark">Eksplorasi Tag: #{{ $activeTag->name }}</h1>
            @else
                <h1 class="text-2xl font-bold text-dark">Eksplorasi</h1>
            @endif
        </div>
            ...
            <div class="flex flex-wrap gap-2 md:max-w-2xl overflow-x-auto hide-scrollbar pb-2">
                @foreach($popularTags as $popTag)
                    <a href="{{ route('search', ['tag' => $popTag->slug]) }}" 
                       class="shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-colors 
                       {{ $activeTag && $activeTag->id === $popTag->id ? 'bg-dark text-white' : 'bg-gray-100 hover:bg-gray-200 text-dark' }}">
                        {{ $popTag->name }}
                    </a>
                @endforeach
            </div>
    ...
    <div id="search-grid" class="w-full -ml-2 sm:-ml-4 relative">
        ...
        @empty
            <div class="col-span-full py-20 flex flex-col items-center w-full text-center pl-4 absolute left-0 right-0">
                ...
                <h2 class="text-xl font-bold text-dark mb-2">Pencarian tidak ditemukan</h2>
                <p class="text-gray-500 max-w-md">Kami tidak dapat menemukan hasil untuk "{{ $query }}". Coba gunakan kata kunci lain atau telusuri tag populer.</p>
            </div>
        @endforelse
    </div>
