...
<div class="w-full max-w-3xl mx-auto pt-12 px-4">
    <div class="flex items-center justify-between mb-10">
        <div>
            <h1 class="text-4xl font-display font-black tracking-tight text-dark dark:text-white">Notifikasi</h1>
            <!-- <p class="text-gray-500 mt-2">Tetap terhubung dengan aktivitas di Bloxpin.</p> -->
        </div>
    ...
    <div class="bg-white dark:bg-card rounded-3xl shadow-minimal border border-borderlight dark:border-borderdark overflow-hidden">
        @if($notifications->count() > 0)
            <div class="divide-y divide-gray-100 dark:divide-borderdark">
                @foreach($notifications as $notification)
                    <div class="p-6 flex items-start gap-4 hover:bg-light dark:hover:bg-dark/20 transition-colors {{ !$notification->read_at ? 'bg-accent/5' : '' }}">
                        <!-- Actor Avatar -->
            ...
            @if($notifications->hasPages())
                <div class="p-6 border-t border-gray-100 dark:border-borderdark">
                    {{ $notifications->links() }}
                </div>
            @endif
        @else
            <div class="py-24 text-center">
                <div class="inline-flex items-center justify-center w-20 h-20 bg-light dark:bg-dark rounded-full mb-4">
                    <svg class="w-10 h-10 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                    </svg>
                </div>
                <h3 class="text-xl font-bold text-dark dark:text-white mb-1">Belum ada notifikasi</h3>
                <p class="text-gray-500">Kami akan memberi tahu Anda jika ada aktivitas baru.</p>
            </div>
        @endif
    </div>
</div>
