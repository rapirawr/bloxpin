...
<div class="max-w-6xl mx-auto px-0 sm:px-4 py-4 md:py-8" x-data="{ 
            ...
            <div class="hidden md:flex justify-between items-center mb-6">
                <div class="flex items-center gap-4">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" class="w-10 h-10 rounded-full hover:bg-light dark:hover:bg-borderdark flex items-center justify-center transition-colors text-dark dark:text-white">
                        ...
                        <div x-show="open" @click.away="open = false" class="absolute top-full left-0 mt-2 w-56 bg-cardlight dark:bg-card rounded-2xl shadow-xl border border-borderlight dark:border-borderdark py-2 z-50 overflow-hidden" style="display: none;">
                            <button @click="
                                navigator.clipboard.writeText(window.location.href);
                                window.showToast('Tautan disalin!');
                                open = false;
                            " class="w-full text-left px-4 py-3 hover:bg-light dark:hover:bg-borderdark flex items-center gap-3 text-dark dark:text-white transition-colors">
                            ...
                            @can('update', $photo)
                                ...
                                <a href="{{ route('photos.edit', $photo) }}" class="w-full text-left px-4 py-3 hover:bg-light dark:hover:bg-borderdark flex items-center gap-3 text-dark dark:text-white transition-colors">
                ...
                <div x-data="{ 
                    ...
                    <!-- Dropdown Boards -->
                    <div x-show="showBoards" @click.away="showBoards = false" class="absolute top-full right-0 mt-2 w-64 bg-cardlight dark:bg-card rounded-xl shadow-minimal dark:shadow-minimal-dark overflow-hidden z-20 border border-borderlight dark:border-borderdark" style="display: none;">
                        <div class="p-3 text-sm font-semibold text-center border-b border-borderlight dark:border-borderdark text-dark dark:text-white">Simpan ke board</div>
                        <div class="max-h-64 overflow-y-auto p-2 scroll-smooth" style="-webkit-overflow-scrolling: touch;">
                            <template x-for="board in boards" :key="board.id">
                                ...
                                " class="w-full text-left px-3 py-2 hover:bg-light dark:hover:bg-borderdark rounded-lg flex items-center justify-between group transition-colors">
                                    <div class="flex items-center gap-3 overflow-hidden">
                                        <div class="w-10 h-10 bg-light dark:bg-borderdark rounded border border-borderlight dark:border-borderdark shrink-0">
                ...
                @if($photo->tags->count() > 0)
                    <div class="flex flex-wrap gap-2 mb-8">
                        @foreach($photo->tags as $tag)
                            <a href="{{ route('search', ['tag' => $tag->slug]) }}" class="px-4 py-2 border border-borderlight dark:border-borderdark hover:bg-light dark:hover:bg-borderdark text-dark dark:text-white rounded-lg text-sm transition-colors font-medium">
                                {{ $tag->name }}
                            </a>
                        @endforeach
                    </div>
                        ...
                        <div class="flex gap-4 mb-8">
                            ...
                            <div class="flex-1 relative">
                                <textarea 
                                    ...
                                    class="w-full bg-light dark:bg-borderdark border-none rounded-2xl py-3 px-4 text-sm focus:ring-2 focus:ring-dark dark:focus:ring-white transition-all resize-none min-h-[44px]"
                                    rows="1"
    ...
    <div x-show="isShareModalOpen" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-0">
        ...
        <div x-show="isShareModalOpen" 
             ...
             class="relative bg-white dark:bg-card rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all z-10 border border-borderlight dark:border-borderdark">
             
             <div class="px-6 py-4 border-b border-borderlight dark:border-borderdark flex items-center justify-between">
                 <h3 class="text-xl font-bold text-dark dark:text-white font-display">Bagikan</h3>
                 <button @click="isShareModalOpen = false" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-light dark:hover:bg-borderdark text-dark dark:text-gray-400 transition-colors">
             ...
             <div class="p-6">
                 ...
                 <div class="mt-4">
                     ...
                     <div class="flex items-center gap-2">
                         <div class="flex-1 bg-light dark:bg-borderdark rounded-xl px-4 py-3 text-sm text-gray-500 dark:text-gray-400 truncate border border-borderlight dark:border-borderdark">
