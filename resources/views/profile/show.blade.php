...
<div class="w-full max-w-7xl mx-auto pt-8">
    ...
    <div class="flex flex-col items-center px-4 mb-10 -mt-16 md:-mt-20 relative z-10">
        ...
        @if($user->bio)
            <p class="text-dark max-w-md text-center mb-6">{{ $user->bio }}</p>
        @endif

        <div class="flex gap-4 items-center font-medium text-dark mb-6">
        ...
        <div class="flex gap-2" x-data="{ 
            ...
            @else
                <button 
                    @click="toggleFollow()" 
                    :class="isFollowing ? 'bg-gray-200 text-dark' : 'bg-dark text-white'"
                ...
                <a href="{{ route('messages.show', $user->username) }}" class="w-11 h-11 bg-light dark:bg-borderdark hover:bg-gray-200 dark:hover:bg-white/10 rounded-full flex items-center justify-center transition-colors text-dark dark:text-white border border-borderlight dark:border-borderdark shadow-sm">
    ...
    <!-- Tabs Navigation -->
    <div class="w-full flex justify-center gap-8 border-b border-gray-100 mb-8 px-4">
        <a href="{{ route('profile.show', ['user' => $user->username, 'tab' => 'created']) }}" 
           class="pb-3 font-semibold text-lg transition-colors relative {{ $tab === 'created' || $tab === '' ? 'text-dark' : 'text-gray-500 hover:text-dark' }}">
            Dibuat
            @if($tab === 'created' || $tab === '')
                <div class="absolute bottom-0 left-0 w-full h-1 bg-dark rounded-t-md"></div>
            @endif
        </a>
        <a href="{{ route('profile.show', ['user' => $user->username, 'tab' => 'saved']) }}" 
           class="pb-3 font-semibold text-lg transition-colors relative {{ $tab === 'saved' ? 'text-dark' : 'text-gray-500 hover:text-dark' }}">
            Disimpan
            @if($tab === 'saved')
                <div class="absolute bottom-0 left-0 w-full h-1 bg-dark rounded-t-md"></div>
            @endif
        ...
        <a href="{{ route('profile.show', ['user' => $user->username, 'tab' => 'boards']) }}" 
           class="pb-3 font-semibold text-lg transition-colors relative {{ $tab === 'boards' ? 'text-dark' : 'text-gray-500 hover:text-dark' }}">
            Board
            @if($tab === 'boards')
                <div class="absolute bottom-0 left-0 w-full h-1 bg-dark rounded-t-md"></div>
            @endif
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    @foreach($boards as $board)
                        <a href="{{ route('boards.show', $board) }}" class="group block">
                            <div class="aspect-square bg-gray-100 rounded-2xl md:rounded-3xl overflow-hidden mb-3 p-1 flex flex-wrap gap-1 relative group-hover:shadow-md transition-all">
                                ...
                                @else
                                    ...
                                    @else
                                        <!-- Empty Board -->
                                        <div class="w-full h-full bg-gray-200 flex items-center justify-center rounded-[20px]"></div>
                                    @endif
                                @endif
                            ...
                            <h3 class="font-bold text-dark text-lg truncate px-1">{{ $board->title }}</h3>
